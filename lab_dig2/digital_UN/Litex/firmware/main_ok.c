	...
	for(j=0; j < 2048; j++){
		for(i = 0; i < 8; i ++){
		  ...
		  if(i==2){
		    ...
		    voltages[j] = 0xffff & data ;// &  ((data & 0xFF)<<8) & (data>>8 & 0xFF);
		  }
		  address ++;
		}
	}
...
int main(void)
{
  ...
  while(0) {
    i2c_write( slave_addr, 0x24, (unsigned char *)0x01, 1, 1 );
    current = ( i2c_read( slave_addr, 0x25, buf, 1, 1, 1) << 8 | i2c_read( slave_addr, 0x26, buf, 1, 1, 1) *20 );
	;
      printf("Current= %d mA\n", current);
      //printf("ID= %x \n", readRegister(I2C_ADDRESS, 0x1));
      //printf("V= %x \n",  readRegister(I2C_ADDRESS, 0x07) );
    my_busy_wait(1000);
  }
