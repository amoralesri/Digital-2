...
> Actualizacion oficial 2026-07-11, Colorlight 5A-75B: se detecto la FPGA por JTAG con `openFPGALoader -c ft232RL --pins=TXD:CTS:DTR:RXD --detect`, `idcode 0x41111043`, fabricante Lattice, familia ECP5, modelo `LFE5U-25`. Se agrego `Litex/colorlight_5a_75b_ws2812_dma.py`, target minimo para 5A-75B con CPU VexRiscv, ROM/RAM integrada, CSR `disp0`, DMA `disp0_dma`, LiteX stream y WS2812 de 64 LEDs. El build para revision asumida `7.0` y pin temporal `j1:0` genero `Litex/build/colorlight_5a_75b_ws2812/gateware/colorlight_5a_75b.bit` y cerro timing post-route a `77.97 MHz (PASS at 60.00 MHz)`, slack setup aproximado `+3.84 ns`. Programacion y matriz fisica siguen bloqueadas hasta confirmar revision impresa y pin fisico real de DIN WS2812.
